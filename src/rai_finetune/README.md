...
### 3. Tool Calling Without Image URLs

The training data formatter automatically handles multimodal content by converting it to text-only format suitable for tool calling fine-tuning. This approach is designed for scenarios where:

- **Tool calling logic is text-based** - The model learns to understand tool definitions, parameters, and responses
- **Images provide context but aren't critical** - Visual information is converted to `[IMAGE]` placeholders
- **Simplified training is preferred** - Text-only models are easier to train and deploy

**Content Normalization Process:**

1. **Text content** is preserved as-is
2. **Image URLs** are replaced with `[IMAGE]` placeholders
3. **Mixed content** (text + images) is flattened to text with placeholders
4. **Tool calls** remain intact for training

**Example Transformation:**

```json
// Input message with mixed content
{
  "role": "user",
  "content": [
    {"type": "text", "text": "Take a photo of the table"},
    {"type": "image_url", "image_url": {...}}
  ]
}

// Output message (normalized)
{
  "role": "user", 
  "content": "Take a photo of the table [IMAGE]"
}
```

This approach ensures that your tool calling model learns the essential patterns while maintaining clean, training-ready data that works with standard chat templates and tokenizers.
