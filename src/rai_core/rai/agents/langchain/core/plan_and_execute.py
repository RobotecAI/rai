...
def create_plan_and_execute_runnable(
    ...
    Make sure that plan can be achieved by the other agent that can use these tools:
    ...
    thinking = ChatOpenAI(model="gpt-4o-mini")
    ...
    critic_llm = critic_prompt | ChatOpenAI(model="gpt-4o-mini").with_structured_output(
        Criticism
    )
    ...
    {plan_str}\n\nYou are tasked with executing first {1} steps, {task}."""
    ...
    async def critic(state: PlanExecute):
        b64_image = camera_tool.invoke(dict())
        msg = HumanMultimodalMessage(
            "Here is the current camera image", images=[b64_image]
        )
        input = {"plan": state["plan"][1:], "image": msg}
