...
class ROS2BaseConnector(ROS2ActionMixin, ROS2ServiceMixin, BaseConnector[T]):
    ...
    def send_message(
        ...
        **kwargs: Any,
        ...
        - LLM support: ROS2Message with dict payload (no imports needed)
        - Typed (human-friendly): Direct ROS2 message classes (type safety, IDE support)

        Use BaseROS2API.dict_to_message() to convert dicts to message classes.

        Parameters
        ----------
        message : T | IROS2Message
            The message to send. Can be a subclass of ROS2Message (payload is a dict)
            or any ROS2 message class instance.
        target : str
            The target topic name.
        msg_type : str | None, optional
            The ROS2 message type. If None, the message type will be inferred from the
            message content. Must be provided if msg_content is a ROS2Message subclass.
        ...
        **kwargs : Any
        ...
        """
        # LLM support path: ROS2Message wrapper with dict payload
        if isinstance(message, ROS2Message):  # T class
            msg_content = message.payload
        # Typed (human-friendly) path: direct ROS2 message class instance
